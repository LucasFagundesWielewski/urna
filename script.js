...
function atualizaInterface() {
    ...
    if (candidato.length > 0) {
        ...
        if (etapaAtual === 0) {
            descricao.innerHTML = `Nome: ${candidato.nome}<br/>Partido: ${candidato.partido}<br/>`; // Exibe detalhes do candidato
        } else {
...
function confirma() {
    ...
    if (votoConfirmado) { // Se o voto foi confirmado
        ...
        if (etapas[etapaAtual] !== undefined) {
            ...
            // Adicionar botão "Próximo Candidato"
            let nextVoteBtn = document.getElementById('next-vote-btn');
            nextVoteBtn.classList.remove('hidden'); // Mostrar o botão
            nextVoteBtn.addEventListener('click', refreshPage); // Adicionar evento de clique
        }
    
        console.log(votos); // Log da lista de votos
    }
    
    // Função para atualizar a página (reiniciar a votação)
